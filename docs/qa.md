...
> 注：上图中"该点不进 Judge（直接拦截）"指**能回绑到评分点**的硬伤（覆盖率 ★/漏答、Judge 旧数据）。
> 数值偏离复核（层 2）转译出的 ★UNDER→BLOCK 属条款级硬伤，当前不携带 `point_id`（数值要求行不与
> 单个评分点绑定），它照常触发 `escalation_required`、置顶报告，但不会掐掉某一评分点的 Judge 复审。
